...
export default function Navbar({ onSave, onLoad, onExportJSON, onExportCSV, savedBuilds, setBuildName, buildName }) {
  ...
  // Save handler with validation and toast
  function handleSave() {
    if (!buildName) {
    ...
    <nav className="w-full bg-white/80 backdrop-blur-md shadow-md border-b border-gray-100 sticky top-0 z-40">
      <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
        ...
        <div className="flex items-center space-x-2 relative">
          ...
          <div
            ref={menuRef}
            className={`transition-all duration-150 ${menuOpen ? 'opacity-100 scale-100 pointer-events-auto' : 'opacity-0 scale-95 pointer-events-none'} absolute right-0 mt-80 w-72 bg-white border rounded shadow-lg z-50`}
            tabIndex={-1}
          >
